...
{
  if (prmr == TRUE) {
    rbind(matrix("", ncol = length(levels(as.factor(x))),
                 dimnames = list(nam)), matrix(c(by(y, x, function(j) {
                   sprintf(paste0("%1.", d, "f (%1.", d, "f) [%1.0f]"),
                           mean(j, na.rm = TRUE), sd(j, na.rm = TRUE), sum(is.na(j)))
                 }), by(y, x, function(k) {
                   sprintf(paste0("%1.", d, "f [%1.", d, "f, %1.", d,
                                  "f]"), median(k, na.rm = T), min(k, na.rm = T),
                           max(k, na.rm = T))
                 })), nrow = 2, byrow = T, dimnames = list(c("Mean (SD) [Missing]",
                                                             "Median [Range]"), levels(as.factor(x)))))
  }
  else {
    rbind(matrix("", ncol = length(levels(as.factor(x))),
                 dimnames = list(nam)), matrix(c(by(y, x, function(j) {
                   sprintf(paste0("%1.", d, "f (%1.", d, "f) [%1.0f]"),
                           mean(j, na.rm = T), sd(j, na.rm = T), sum(is.na(j)))
                 })), nrow = 1, byrow = T, dimnames = list(c("Mean (SD) [Missing]"),
                                                           levels(as.factor(x)))))
  }
}
